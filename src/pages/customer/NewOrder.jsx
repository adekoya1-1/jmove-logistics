...
export default function NewOrder() {
   ...
   * Creates the order on the backend (idempotent) and, for online
   * payment, initialises a Paystack transaction to obtain an access_code.
   * The amount is fully determined by the backend — not by anything
   * the frontend sends at this point.
   */
  const confirmQuote = async () => {
    ...
    try {
      ...
      if (form.paymentMethod === 'online') {
        /*
         * Initialise payment on backend.
         * The backend calls Paystack and returns an access_code that is
         * bound to the exact amount we calculated. The frontend passes
         * this opaque token to the Paystack SDK — no amount manipulation
         * is possible at this stage.
         */
        const payRes = await paymentsAPI.initialize(order._id);
        setPaymentInit({
          reference:         payRes.data.reference,
          access_code:       payRes.data.access_code,
          authorization_url: payRes.data.authorization_url, // redirect fallback
        });
        transition(S.PAYING);
      } else {
        /* Cash / COD — no online payment step */
        transition(S.BOOKED);
      }
    } catch (e) {
      setError(
  ...
  /**
   * Step 4 — Open Paystack Inline popup
   * Uses access_code so the amount is server-controlled and cannot be
   * altered by the frontend. PaystackPop.newTransaction() does NOT need
   * a public key when an access_code is provided.
   */
  const openPaystackPopup = async () => {
    if (!paymentInit?.access_code) {
    ...
    try {
      ...
      // Loading spinner can be dismissed now — the popup takes over the UI
      setLoading(false);
      ...
      PaystackPop.newTransaction({
        ...
        onSuccess: (tx) => {
          /*
           * Paystack reports success client-side.
           * IMPORTANT: we do NOT trust this alone. We immediately ask
           * our backend to verify the transaction with Paystack's API.
           * Only after backend verification do we mark the booking confirmed.
           */
          setMachineState(S.VERIFYING);
          verifyPayment(tx.reference || paymentInit.reference);
        },

        onCancel: () => {
          /*
           * User closed the popup without paying. Stay on PAYING state
           * so they can retry. The access_code remains valid for ~10 min.
           */
          setLoading(false);
          ...
          <div className="order-step fade-in">
            ...
            {machineState === S.VERIFYING ? (
              ...
              <>
                ...
                {/* Paystack CTA — this is the payment "form" */}
                <button
